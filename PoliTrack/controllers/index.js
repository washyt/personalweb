...
router.use(require('../controllers/forum'));
router.use(require('../controllers/bills'));
router.use(require('../controllers/comments'));
router.use(require('../controllers/upload'));
router.use(require('../controllers/profile'));
router.use(require('../controllers/chat'));
...
{
  if(request.body.ask == ("Save (keep consistent billname)")){  Note.editNote(userID,name,date,notes,names);
    response.status(200);
    response.setHeader('Content-Type', 'text/html')
    response.render("home", {
      "data": JSON.parse(newsdata),
